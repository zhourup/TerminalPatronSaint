package com.zhourup.guard.manage.service.impl;

import com.zhourup.common.utils.Query;
import org.springframework.stereotype.Service;
import java.util.Map;
import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.core.metadata.IPage;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.zhourup.common.utils.PageUtils;

import com.zhourup.guard.manage.dao.PreloadDao;
import com.zhourup.guard.manage.entity.PreloadEntity;
import com.zhourup.guard.manage.service.PreloadService;


@Service("preloadService")
public class PreloadServiceImpl extends ServiceImpl<PreloadDao, PreloadEntity> implements PreloadService {

    @Override
    public PageUtils queryPage(Map<String, Object> params) {
        IPage<PreloadEntity> page = this.page(
                new Query<PreloadEntity>().getPage(params),
                new QueryWrapper<PreloadEntity>()
        );

        return new PageUtils(page);
    }

}